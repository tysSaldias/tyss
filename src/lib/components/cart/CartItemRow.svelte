...
<div class="relative flex items-center gap-3 rounded-xl bg-gray-800/50 p-3 pr-9">
	<!-- Delete button (top-right corner) -->
	<button
		onclick={() => onDelete(line.key)}
		aria-label={`Eliminar ${line.name}`}
		class="absolute right-2 top-2 rounded-md p-1 text-gray-500 transition-colors hover:bg-red-500/20 hover:text-red-400"
	>
	...
	<!-- Line total -->
	<span class="shrink-0 text-sm font-bold text-brand-yellow">
		{priceFormat(line.unitPrice * line.quantity)}
	</span>
</div>
