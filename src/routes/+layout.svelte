<script lang="ts">
	...
	import { items, isHydrated, markHydrated, openSheetSignal } from '$lib/stores/cart.svelte';
	...
	// Auto-open CartSheet when addToCart fires on mobile.
	$effect(() => {
		const _sig = openSheetSignal();
		if (_sig > 0) sheetOpen = true;
	});
